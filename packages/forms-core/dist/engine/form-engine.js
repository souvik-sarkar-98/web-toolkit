...
import { buildDefByKey, getParentStringValue, isFieldVisible, } from './visibility.js';
import { matchesValidationPattern, validationErrorMessageForValue, } from './validation-pattern.js';
import { dateConstraintErrorMessage, isDateRangeWithinBounds, isIsoDateWithinBounds, resolveEffectiveDateBounds, } from './date-constraints.js';
export function resolveFieldOptions(def, values) {
    const { fieldOptions } = def;
    return typeof fieldOptions === 'function' ? fieldOptions(values) : fieldOptions;
}
function resolveAvailableOptions(def, defByKey, values) {
...
export function resolveFieldState(def, defByKey, values, userPermissions = [], ignoreEncrypted = false) {
...
export function resolveAllFields(definition, values, options = {}) {
...
export function groupFieldsByStep(resolved) {
...
function valueAllowedForOptions(def, value, availableKeys) {
    if (value === null || value === undefined || availableKeys.size === 0)
        return true;
...
export function validateForm(definition, values, options = {}) {
    const resolved = resolveAllFields(definition, values, options);
    const missingMandatory = [];
    const conditionViolations = [];
    ...
    for (const field of resolved) {
        ...
        if (!isEmptyValue(fieldType, value, options) &&
            validationRules &&
            !matchesValidationPattern(fieldType, value, validationRules, options)) {
            validationViolations.push(key);
            fieldErrors[key] = validationErrorMessageForValue(fieldType, label, value, validationRules, options);
            continue;
        }
        ...
        if (!isEmptyValue(fieldType, value, options) &&
            ...
            const withinBounds = fieldType === 'date'
...
export function applyDependentValueEffects(definition, values, changedKey, engineOptions = {}) {
    const next = { ...values };
    const defByKey = buildDefByKey(definition.fields);
    const resolved = resolveAllFields(definition, next);
    for (const field of resolved) {
        const def = field.definition;
        const dependsOnChangedKey = def.dependentOptions?.dependsOnKey === changedKey ||
...
function getDefaultForClear(fieldType) {
    switch (fieldType) {
        case 'boolean':
        case 'toggle':
            return false;
...
//# sourceMappingURL=form-engine.js.map